...
public class ZIPUtil {
    ...
    public static byte[] createZIP(Map<String, List<UserDocument>> documentMap) {
        try (ByteArrayOutputStream byteArrayOut = new ByteArrayOutputStream();
            ...
            for (String path : documentMap.keySet()) {
                for (UserDocument doc : documentMap.get(path)) {
                    zipOut.putNextEntry(new ZipEntry(path + doc.getName()));
